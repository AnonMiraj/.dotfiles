...
const elevate = userOptions.asyncGet().etc.widgetCorners
    ? "osd-round osd-bg osd-colorscheme"
    : "osd-bg osd-colorscheme elevation";
...
function calculateSchemeInitIndex(optionsArr, searchValue = "content") {
    ...
    const result = flatArray.findIndex((element) =>
...
export const LIGHTDARK_FILE_LOCATION =
    `${GLib.get_user_state_dir()}/ags/user/colormode.txt`;
...
export const initTransparencyModeVal = (initTransparencyMode == "intense")
    ? 1
    : 0;
...
// initGowall and initGowallIndex removed
    ...
    Widget.Box({
        ...
        children: [
            Widget.Box({
                ...
                children: [
                    ...
                    ConfigToggle({
                        ...
                        onChange: async (self, newValue) => {
                            try {
                                const transparencyMode = newValue == 0
                                    ? "normal"
                                    : "intense";
                                await execAsync([
                                    `bash`,
            ...
            Widget.Box({
                vertical: true,
                spacing: 10,
                css: `margin-bottom:1.5rem`, // Added margin here to compensate for removed element
                children: [
                    Widget.Label({
                        xalign: 0,
                        className: "txt-norm titlefont onSurfaceVariant",
                    ...
                    ConfigMulipleSelection({
                        ...
                        onChange: async (value, name) => {
                            ...
                            ]), runMatugen(); // Assuming runMatugen() is defined elsewhere or should be App.applyCss or similar
                        },
                    }),
                    // Wallpaper Styles Label and ConfigMultipleSelection removed
                    //////////////////
                ],
            }),
        ],
    });
const topLeftCorner = userOptions.asyncGet().etc.widgetCorners
    ? RoundedCorner("topleft", {
        className: "corner corner-colorscheme",
    })
    : null;
const topRightCorner = userOptions.asyncGet().etc.widgetCorners
                    ...
                    Widget.Box({
                        ...
                        children: [
                            ...
                            ColorBox({
                                name: "P-c",
                                className:
                                    "osd-color osd-color-primaryContainer",
                            }),
                            ColorBox({
                                name: "S-c",
                                className:
                                    "osd-color osd-color-secondaryContainer",
                            }),
                            ColorBox({
                                name: "T-c",
                                className:
                                    "osd-color osd-color-tertiaryContainer",
                            }),
                            ColorBox({
                                name: "Sf-c",
                                className:
                                    "osd-color osd-color-surfaceContainer",
                            }),
                            ColorBox({
                                name: "Sf-v",
                ...
                .hook(showColorScheme, (revealer) => {
                    if (showColorScheme.value == true) {
                        ...
                        revealer.revealChild =
                            isHoveredColorschemeSettings.value;
                    }
                })
