        ...
        const imageBox = Box({
            ...
            attribute: {
                "update": (self, data, force = false) => {
                    const imagePath =
                        `${USER_CACHE_DIR}/ags/media/waifus/${data.md5}.${data.file_ext}`;
                    ...
                    // const widgetWidth = Math.min(Math.floor(booruContent.get_allocated_width() * 0.9 / 2), data["preview_width"]);
                    const widgetWidth = Math.floor(
                        booruContent.get_allocated_width() * 0.9 / 2,
                    );
                    ...
                    const showImage = () => {
                        ...
                        imageArea.connect("draw", (widget, cr) => {
                            const borderRadius = widget.get_style_context()
                                .get_property(
                                    "border-radius",
                    ...
                    const downloadCommand =
                        `curl -L -o '${imagePath}' '${data.preview_url}'`;
                    if (!force && fileExists(imagePath)) showImage();
                    else {Utils.timeout(delay, () =>
                ...
                children: [
                    ...
                    ImageAction({
                        name: `${getString("Go to source")} (${
                            getDomainName(data.source)
                        })`,
                        icon: "open_in_new",
                        action: () =>
                            execAsync(["xdg-open", `${data.source}`]).catch(
                                print,
                            ),
                    }),
                    ImageAction({
                        ...
                        icon: "save",
                        action: (self) => {
                            const currentTags = BooruService.queries.at(-1)
                                .realTagList.filter((tag) =>
                                    !tag.includes("rating:")
                                );
                            const tagDirectory = currentTags.join("+");
                            const fileName = decodeURIComponent(
                                data.file_url.substring(
                                    data.file_url.lastIndexOf("/") + 1,
                                ),
                            );
                            const saveCommand =
                                `mkdir -p "$(xdg-user-dir PICTURES)/homework/${
                                    data.is_nsfw ? "🌶️/" : ""
                                }" && curl -L -o "$(xdg-user-dir PICTURES)/homework/${
                                    data.is_nsfw ? "🌶️/" : ""
                    ...
                    ImageAction({
                        ...
                        action: (self) => {
                            ...
                            const saveCommand =
                                `mkdir -p "$(xdg-user-dir PICTURES)/Wallpapers" && curl -L -o "$(xdg-user-dir PICTURES)/Wallpapers/${fileName}" '${data.file_url}'`;
                            const setWallpaperCommand =
                                `${App.configDir}/scripts/color_generation/wallpapers.sh -S "$(xdg-user-dir PICTURES)/Wallpapers/${fileName}"`;
                            // const
                            execAsync([
                                "bash",
                                "-c",
        ...
        attribute: {
            ...
            "update": (data, force = false) => {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    pageImages.children[minIndex].attribute.height += 1 /
                        data[i].aspect_ratio; // we want height/width
                }
                pageImages.show_all();
    ...
    setup: (self) =>
        self
            ...
            .hook(BooruService, (box, id) => {
                ...
                box.attribute.map.get(id)?.attribute.update(
                    BooruService.responses[id],
                );
            }, "updateResponse"),
});

export const BooruView = (chatEntry) =>
    Scrollable({
...
export const booruCommands = Box({
    ...
    setup: (self) => {
        ...
        self.pack_end(
            CommandButton("/mode konachan", "Konachan"),
            false,
            false,
            0,
        );
        self.pack_end(
            CommandButton("/mode yandere", "yande.re"),
            false,
            false,
            0,
        );
