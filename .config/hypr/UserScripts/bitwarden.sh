    ...
    case $1 in
        windowtitle*)
            ...
            if [[ "$window_title" == *"(Bitwarden"*"Password Manager) - Bitwarden"* ]]; then
                ...
                hyprctl --batch "dispatch togglefloating address:0x$window_id; dispatch pin address:0x$window_id; dispatch resizewindowpixel exact 20% 54%,address:0x$window_id; dispatch centerwindow"
            fi
            ;;
    esac
}
